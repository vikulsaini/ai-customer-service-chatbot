  ...
  return (
    <div className="grid gap-4">
      <header className="glass rounded-lg p-4 sm:p-5">
        <h1 className="text-2xl font-bold sm:text-3xl">Chat History</h1>
        <p className="mt-2 text-sm text-slate-600 dark:text-slate-300 sm:text-base">Search, review, and delete user-specific support conversations.</p>
      </header>
      ...
      <section className="glass rounded-lg p-4">
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          <Button onClick={loadChats} className="w-full sm:w-auto">Search</Button>
        </div>
      ...
      <section className="grid gap-3">
        ...
        {chats.map((chat) => (
          <article key={chat._id} className="glass rounded-lg p-4 sm:p-5">
            <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
              <div className="min-w-0">
                <h2 className="break-words text-lg font-bold sm:text-xl">{chat.title}</h2>
              ...
              <button aria-label="Delete chat" onClick={() => deleteChat(chat._id)} className="grid h-11 w-11 shrink-0 place-items-center self-start rounded-lg bg-rose-600 text-white"><Trash2 size={18} /></button>
            </div>
            <div className="mt-4 grid gap-2">
              {(chat.messages || []).slice(-4).map((message) => (
                <p key={message._id || `${message.role}-${message.timestamp}`} className="break-words rounded-lg bg-white/55 p-3 text-sm dark:bg-white/10">
