    ...
    <main className="min-h-screen overflow-hidden bg-[#edf7f5] text-slate-950 dark:bg-[#07111f] dark:text-white">
      <section className="relative min-h-screen px-5 py-6">
        ...
        <nav className="relative z-10 mx-auto flex max-w-7xl items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3 font-bold"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-ocean text-white"><Bot size={21} /></span><span className="truncate">AI Customer Service Chatbot</span></div>
          <div className="flex shrink-0 gap-2"><Link to="/login"><Button variant="ghost" className="px-3 sm:px-4">Login</Button></Link><Link to="/signup"><Button className="px-3 sm:px-4">Signup</Button></Link></div>
        </nav>
        <div className="relative z-10 mx-auto grid min-h-[calc(100vh-96px)] max-w-7xl items-center gap-10 py-10 lg:grid-cols-[1.02fr_.98fr]">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h1 className="max-w-4xl text-4xl font-black leading-tight tracking-normal sm:text-5xl md:text-7xl">AI Customer Service Chatbot</h1>
            <p className="mt-5 max-w-2xl text-base leading-7 text-slate-700 dark:text-slate-300 sm:text-lg sm:leading-8">A full-stack IT support platform with JWT auth, OpenAI-powered answers, NLP intent detection, tickets, chat history, and admin analytics.</p>
