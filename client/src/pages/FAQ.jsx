  ...
  return (
    <div className="grid gap-4">
      <header className="glass rounded-lg p-4 sm:p-5">
        <h1 className="flex items-center gap-3 text-2xl font-bold sm:text-3xl"><HelpCircle className="shrink-0" />FAQ Support</h1>
        <p className="mt-2 text-sm text-slate-600 dark:text-slate-300 sm:text-base">Instant answers for common IT-sector customer service questions.</p>
      </header>
      ...
      <section className="glass rounded-lg p-4">
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          <Button onClick={loadFaqs} className="w-full sm:w-auto">Search</Button>
        </div>
      </section>
      ...
      <section className="grid gap-3 lg:grid-cols-2">
        ...
        {faqs.map((faq) => (
          <article key={faq._id} className="glass rounded-lg p-4 sm:p-5">
            <span className="rounded-full bg-ocean/10 px-3 py-1 text-xs font-bold uppercase text-ocean">{faq.category}</span>
            <h2 className="mt-4 break-words text-lg font-bold sm:text-xl">{faq.question}</h2>
            <p className="mt-3 break-words text-sm leading-7 text-slate-600 dark:text-slate-300 sm:text-base">{faq.answer}</p>
          </article>
        ))}
      </section>
    </div>
